...
static __forceinline__ __device__ float3 add3(const float3 &a,
                                              const float3 &b) {
...
static __forceinline__ __device__ float3 madd3(const float3 &a, float s,
                                               const float3 &b) {
...
static __forceinline__ __device__ uint32_t pcg32(uint32_t &state) {
  ...
  uint32_t word =
      ((oldState >> ((oldState >> 28) + 4)) ^ oldState) * 277803737u;
  state = (word >> 22) ^ word;
  return state;
}
...
static __forceinline__ __device__ Ray generateRay(const OptixCameraData &camera,
                                                  uint2 pixel,
                                                  uint2 resolution) {
  const float3 forward = normalize3(make_float3(
      camera.lookAt.x - camera.lookFrom.x, camera.lookAt.y - camera.lookFrom.y,
      camera.lookAt.z - camera.lookFrom.z));
  ...
  const float px = (2.0f * ((static_cast<float>(pixel.x) + 0.5f) /
                            static_cast<float>(resolution.x)) -
                    1.0f) *
                   tanf(camera.vfov * 0.5f) * aspect;
  const float py = (1.0f - 2.0f * ((static_cast<float>(pixel.y) + 0.5f) /
                                   static_cast<float>(resolution.y))) *
                   tanf(camera.vfov * 0.5f);

  Ray ray;
  ray.origin = camera.lookFrom;
  ray.direction =
      normalize3(add3(add3(forward, mul3(px, right)), mul3(py, up)));
  return ray;
}
...
static __forceinline__ __device__ float3 trace(const Ray &ray, float time) {
  ...
  optixTrace(optixLaunchParams.scene, ray.origin, ray.direction, 0.0f, 1e16f,
             time, 0xFF, OPTIX_RAY_FLAG_DISABLE_ANYHIT, 0, 1, 0, p0, p1, p2);

  return make_float3(__uint_as_float(p0), __uint_as_float(p1),
                     __uint_as_float(p2));
}
...
extern "C" __global__ void __raygen__motion_blur() {
  ...
  const uint2 resolution =
      make_uint2(optixLaunchParams.width, optixLaunchParams.height);

  Ray ray = generateRay(optixLaunchParams.camera, pixel, resolution);

  float3 color = make_float3(0.0f, 0.0f, 0.0f);
  // for jitter uint32_t seed = pixel.y * resolution.x + pixel.x;
  ...
  if (optixLaunchParams.flipY) {
    writeIndex =
        (optixLaunchParams.height - 1u - pixel.y) * resolution.x + pixel.x;
  }

  optixLaunchParams.colorBuffer[writeIndex] =
...
extern "C" __global__ void __closesthit__motion() {
  ...
  const float3 color =
      make_float3(rand_float(seed), rand_float(seed), rand_float(seed));
  storePayload(color);
}
