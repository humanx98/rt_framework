...
#define CUDA_CHECK(call)                                                                                            \
    do {                                                                                                            \
        ...
        if (_cuda_result != CUDA_SUCCESS) {                                                                         \
            ...
            std::println("[cuda][error] {}: {}", errName ? errName : "Unknown", errString ? errString : "No description"); \
            std::abort();                                                                                           \
        }                                                                                                           \
    } while (0)
...
#define OPTIX_CHECK(call)                                                                                           \
    do {                                                                                                            \
        ...
        if (_optix_result != OPTIX_SUCCESS) {                                                                       \
            ...
            std::println("[optix][error] {}: {}", errName ? errName : "Unknown", errString ? errString : "No description"); \
        }                                                                                                           \
    } while (0)
...
OptixRenderer::OptixRenderer(RenderSession* session)
    : m_session(session)
{}

OptixRenderer::~OptixRenderer() {
    if (d_launchParams) CUDA_CHECK(cuMemFree(d_launchParams));
    if (d_output) CUDA_CHECK(cuMemFree(d_output));

    if (d_raygenRecord) CUDA_CHECK(cuMemFree(d_raygenRecord));
    if (d_missRecord) CUDA_CHECK(cuMemFree(d_missRecord));
    if (d_hitRecord) CUDA_CHECK(cuMemFree(d_hitRecord));

    for (auto& mesh : meshes) {
        if (mesh.vertices) CUDA_CHECK(cuMemFree(mesh.vertices));
        if (mesh.indices) CUDA_CHECK(cuMemFree(mesh.indices));
        if (mesh.gasBuffer) CUDA_CHECK(cuMemFree(mesh.gasBuffer));
    }

    for (auto& motion : motionTransforms) {
        if (motion.devicePtr) CUDA_CHECK(cuMemFree(motion.devicePtr));
    }

    if (tlasBuffer) CUDA_CHECK(cuMemFree(tlasBuffer));
    ...
    if (cudaContext) CUDA_CHECK(cuCtxDestroy(cudaContext));
}
...
//     size_t pixelCount = static_cast<size_t>(resolution.x) * static_cast<size_t>(resolution.y);
//     if (out.size() < pixelCount) {
//         out.resize(pixelCount);
//     }

//     CUDA_CHECK(cuMemcpyDtoH(out.data(), d_output, pixelCount * sizeof(glm::vec4)));
// }
...
void OptixRenderer::createModuleAndPipeline() {
    const std::filesystem::path ptxPath = resolveDevicePath(rtf::RenderBackend::Optix, "OptixMotionBlur.ptx");
...
void OptixRenderer::createProgramGroups() {
    ...
    OPTIX_CHECK(optixProgramGroupCreate(optixContext, &raygenDesc, 1, &programGroupOptions, nullptr, nullptr, &raygenPG));

    OptixProgramGroupDesc missDesc = {};
    missDesc.kind = OPTIX_PROGRAM_GROUP_KIND_MISS;
    missDesc.miss.module = module;
    missDesc.miss.entryFunctionName = "__miss__constant";
    OPTIX_CHECK(optixProgramGroupCreate(optixContext, &missDesc, 1, &programGroupOptions, nullptr, nullptr, &missPG));
    ...
    OPTIX_CHECK(optixProgramGroupCreate(optixContext, &hitDesc, 1, &programGroupOptions, nullptr, nullptr, &hitPG));
}
...
void OptixRenderer::buildMeshes(const Scene& scene) {
    ...
    for (const TriangleMesh& mesh : scene.meshes) {
        ...
        CUDA_CHECK(cuMemcpyHtoD(buffers.vertices, mesh.vertices.data(), vertexBytes));

        CUDA_CHECK(cuMemAlloc(&buffers.indices, indexBytes));
        CUDA_CHECK(cuMemcpyHtoD(buffers.indices, mesh.triangles.data(), indexBytes));
        ...
        triangles.numIndexTriplets = static_cast<unsigned int>(mesh.triangles.size());
        ...
        OPTIX_CHECK(optixAccelComputeMemoryUsage(optixContext, &buildOptions, &buildInput, 1, &bufferSizes));
...
void OptixRenderer::createMotionTransform(const Instance& instance, MotionTransform& motion) {
    ...
    matrixTransform.motionOptions.numKeys = static_cast<unsigned int>(instance.transforms.size());
    ...
    for (size_t key = 0; key < instance.transforms.size(); ++key) {
        writeOptixMatrix(instance.transforms[key].matrix, matrixTransform.transform[key]);
    }

    CUDA_CHECK(cuMemAlloc(&motion.devicePtr, sizeof(OptixMatrixMotionTransform)));
    CUDA_CHECK(cuMemcpyHtoD(motion.devicePtr, &matrixTransform, sizeof(matrixTransform)));
...
void OptixRenderer::buildInstances(const Scene& scene) {
    ...
    for (size_t i = 1; i < scene.instances.size(); ++i) {
        ...
        writeOptixMatrix(scene.instances.front().transforms.front().matrix, optixInstance.transform);
        //std::memcpy(optixInstance.transform, , sizeof(optixInstance.transform));
...
void OptixRenderer::allocateOutputBuffer() {
    const size_t pixelCount = static_cast<size_t>(resolution.x) * static_cast<size_t>(resolution.y);
