...
void optixLogCallback(unsigned int level, const char *tag, const char *message,
                      void *) {
  std::println("[optix][{}][{}] {}", level, tag ? tag : "",
               message ? message : "");
}
...
std::filesystem::path resolveDevicePath(rtf::RenderBackend backend,
                                        std::string filename) {
  static const std::array<const char *, 3> optixCandidates = {
      "../cuda/device", "cuda/device", "../../src/rt_framework/cuda/device"};

  static const std::array<const char *, 3> hiprtCandidates = {
      "../hip/device", "hip/device", "../../src/rt_framework/hip/device"};

  const auto &candidates = (backend == rtf::RenderBackend::Optix)
                               ? optixCandidates
                               : hiprtCandidates;
